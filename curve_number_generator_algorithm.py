...
class CurveNumberGeneratorAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(QgsProcessingParameterVectorLayer('areaboundary', 'Area Boundary', types=[
                          QgsProcessing.TypeVectorPolygon], defaultValue=None))
        param = QgsProcessingParameterFeatureSource('cnlookup', 'CN_Lookup.csv', optional=True, types=[
                                                    QgsProcessing.TypeVector], defaultValue='')
        param.setFlags(
            param.flags() | QgsProcessingParameterDefinition.FlagAdvanced)
        self.addParameter(param)
        param = QgsProcessingParameterBoolean(
            'drainedsoilsleaveuncheckedifnotsure', 'Drained Soils? [leave unchecked if not sure]', defaultValue=False)
        param.setFlags(
            param.flags() | QgsProcessingParameterDefinition.FlagAdvanced)
        self.addParameter(param)
        self.addParameter(QgsProcessingParameterBoolean(
            'OutputNLCDLandCoverRaster', 'Output NLCD Land Cover Raster', defaultValue=False))
        self.addParameter(QgsProcessingParameterBoolean(
            'OutputNLCDLandCoverVector', 'Output NLCD Land Cover Vector', defaultValue=False))
        self.addParameter(QgsProcessingParameterBoolean(
            'OutputSoilLayer', 'Output Soil Layer', defaultValue=False))
        self.addParameter(QgsProcessingParameterBoolean(
            'OutputCurveNumberLayer', 'Output Curve Number Layer', defaultValue=False))
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        soil_output = self.parameterAsBool(
            parameters, 'OutputSoilLayer', context)
        ...
        if parameters['cnlookup'] == None:
            csv_uri = "file:///" + \
                os.path.join(cmd_folder, 'CN_Lookup.csv')+"?delimiter=,"
        ...
        extent_area = d.measureArea(
            QgsGeometry().fromRect(area_layer.extent()))
        area_acres = d.convertAreaMeasurement(
            extent_area, QgsUnitTypes.AreaAcres)
        ...
        if curve_number_output == True or nlcd_vect_output == True or nlcd_rast_output == True:
            ...
            request_URL = 'https://www.mrlc.gov/geoserver/mrlc_display/NLCD_2016_Land_Cover_L48/ows?version=1.3.0&service=WMS&layers=NLCD_2016_Land_Cover_L48&styles&crs=' + \
                str(EPSGCode) + '&format=image/geotiff&request=GetMap&width=' + str(BBOX_width_int) + '&height=' + \
                str(BBOX_height_int) + '&BBOX=' + str(xmin) + ',' + \
            ...
            outputs['ReclassifyByTable'] = processing.run(
                'native:reclassifybytable', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
            ...
            try:  # for QGIS Version later than 3.12
                ...
                feedback.reportError(
                    'You are not using the latest version of QGIS, defaulting to the old version of native algorithm \'set style layer\'.'+'\n', True)
                outputs['SetStyleForRasterLayer'] = processing.run(
                    'qgis:setstyleforrasterlayer', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
            ...
            if curve_number_output == True or nlcd_vect_output == True:
                ...
                try:  # for QGIS Version 3.12 and later
                    ...
                    outputs['SetStyleForVectorLayer'] = processing.run(
                        'qgis:setstyleforvectorlayer', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        if soil_output == True or curve_number_output == True:
            ...
            request_URL_soil = 'https://sdmdataaccess.sc.egov.usda.gov/Spatial/SDMWGS84GEOGRAPHIC.wfs?SERVICE=WFS&VERSION=1.1.0&REQUEST=GetFeature&TYPENAME=mapunitpolyextended&SRSNAME=EPSG:4326&BBOX=' + \
                str(xmin_reprojected) + ',' + str(ymin_reprojected) + \
                ',' + str(xmax_reprojected) + ',' + str(ymax_reprojected)
            ...
            outputs['SwapXAndYCoordinates'] = processing.run(
                'native:swapxy', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
